...
const elplisCore = require('../../elpis-core');
const project = require('../../app/router/project.js');
...
// 测试用例
describe('c测试 project 相关接口', function() {
    this.timeout(60000);
    ...
    it('GET /api/project without projKey', async () => {
    ...
    it('GET /api/project fail', async () => {
        ...
        tmpRequest = tmpRequest.set('s_t', st);
        tmpRequest = tmpRequest.set('s_sign', md5(`${signKey}_${st}`));
        tmpRequest = tmpRequest.query({ proj_key: 'xxxqq' });
        const res = await tmpRequest;
        assert(res.body.success === false);
    ...
    it('GET /api/project with projKey', async () => {
        ...
        for (let i = 0; i < projectList.length; i++) {
            ...
            console.log(projKey, 'projKey projKey');
        ...
        function checkModule(menuItem) {
            console.log(menuItem.key, '`````````````````checkModule checkModulemenuItem menuItem`````````````````');
            ...
            if (moduleType === 'sider') {
                ...
                siderConfig.menu.forEach(siderMenuItem => {
                    assert(siderMenuItem.key);
                    assert(siderMenuItem.name);
                    assert(siderMenuItem.menuType);
                    assert(siderMenuItem.menuType === 'module');
    ...
    it('GET /api/project/list withoutt proj_Key', async () => {
